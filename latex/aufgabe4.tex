...
Um die Werte zu bestimmen wurden unterschiedliche Methoden verwendet um die Bauteilgrößen zu bestimmen.

\begin{minipage}[t]{0.49\textwidth}
    \textbf{Widerstand}:
    \begin{itemize}[nolistsep, noitemsep]
        \item Ablesen des Farbcodes und Vergleich mit der Wertetabelle.
        \item Messung von Strom und Spannung wie in \autoref{fig:ui-mess} (c) und (d) gezeigt.
        \item Spannungsteiler Messung mit dem ADC des Arduinos.
    \end{itemize}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\textwidth}
    \textbf{Kondensator}:
    \begin{itemize}[nolistsep, noitemsep]
        \item Ablesen der Kapazität am Bauteil.
        \item Messung der Ladezeit über den ADC des Arduinos.
    \end{itemize}
\end{minipage}
...
Vor dem Aufbau wurde eine Prototyp-Schaltung in Tinker-CAD\footnote{\href{https://www.tinkercad.com/things/6dP6bOFcvVJ-4-widerstandsmessung?sharecode=cloxrV9fQVUj9Cfl4C-4CEYhAw4g13SRQi1Q5vM9bJg}{Tinker CAD Share Link}} aufgebaut und emuliert, um die Durchführung am Steckbrett zu erleichtern. Im Anschluss wurde die Schaltung am Steckbrett aufgebaut.
...
\begin{subfigure}[b]{0.3\textwidth}
    \centering
    \includegraphics[width=.9\textwidth]{latex/images/fotos/4_a.JPEG}
...
Vor dem Aufbau wurde eine Prototyp-Schaltung in Tinker-CAD\footnote{\href{https://www.tinkercad.com/things/84XtVuCZHAm-4-kapazitatsmessung?sharecode=4kCbAaRwm3v1nCWt6jAq8217Vbh7pe0KWzrzvLyUI0Q}{Tinker CAD Share Link}} aufgebaut und emuliert, um die Durchführung am Steckbrett zu erleichtern. Im Anschluss wurde die Schaltung am Steckbrett aufgebaut.
...
\begin{subfigure}[b]{0.3\textwidth}
    \centering
    \includegraphics[width=.9\textwidth]{latex/images/fotos/4_b.JPEG}
    \caption{Verdrahtung am Steckbrett}
\end{subfigure}
\end{figure}
...
\subsubsection{Implementierung der Kapazitätsmessung}

\lstinputlisting[
    lastline=9,
    caption={Globale Variablen und Definitionen}
]{../04-Widerstand/cap/cap.ino}
...
\subsubsection{Fazit der unterschiedlichen Messmethoden}

\paragraph{Widerstände} Es sind deutliche Unterschiede feststellbar. Während Farbcode und Multimeter konsistente Werte liefern, weicht die Mikrocontroller-Messung teilweise stark ab. Besonders bei sehr kleinen Widerständen (z. B. \SI{100}{\ohm}) ist der Fehler massiv, während er bei Werten nahe dem Referenzwiderstand (\SI{12}{\kilo\ohm}) geringer ist. Die Abweichungen liegen primär in der Auflösung des ADCs (10-Bit) und der Dimensionierung des Spannungsteilers:
\begin{itemize}
    \item Quantisierungsfehler: Bei kleinen Widerständen ($R_x \ll R_{Ref}$) ist die Messspannung extrem niedrig. Der ADC nutzt nur \SI{10}{\bit}, wodurch bereits ein Fehler von $\pm 1$ Bit zu enormen prozentualen Abweichungen führt.
    \item Spannungsteiler-Prinzip: Die Messung ist nur präzise, wenn $R_x$ in einer ähnlichen Größenordnung wie $R_{Ref}$ ($\approx$ \SI{10}{\kilo\ohm}) liegt.
\end{itemize}

\paragraph{Kondensatoren} Bei der Kondensatoren konnte eine geringe Abweichung zwischen Messung und Marking Code festgestellt werden. Durch die hohe Toleranz von \SI{10}{\percent} wird jedoch die Messung durch das Programm genauer sein als der angegebene Kondensator.